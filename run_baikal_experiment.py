...
EXPERIMENT_OUT_DIR = os.path.join(
    ...
    'compare_imgs', '10_presentation_10.01'
)
...
HYBRID_ARGS = f'-w {RESOLUTION[0]} -h {RESOLUTION[1]} --rt_max_recursion={NUM_BOUNCE} ' \
...
def run_experiment(config: ExperimentConfig, standalone_path: str):
    ...
    experiment_dir_path = os.path.join(standalone_dir_path, EXPERIMENT_OUT_DIR, dir_name)
    ...
    hybrid_process = subprocess.Popen(
        f'{standalone_path} --ifn={image_name} {HYBRID_ARGS}',
