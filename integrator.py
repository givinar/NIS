...
class Integrator:
    ...
    def train_one_step(self, nsamples, lr=None, points=False, integral=False):
        ...
        # loss = (
        #         (y/absdet) * (torch.log(y) - absdet)
        # ).mean()
        # loss = (
        #         (y* torch.log(y)) - (y * absdet)
        # ).mean()
        # loss = (y * torch.log(y)).mean() - (y * absdet).mean()
        # z = z.cpu()
        # loss = torch.sum(y - absdet)
        # loss = (y - absdet).mean(0)
        # y = y / y.max()
        ...
        if integral:
            ...
            return_dict["uncertainty"] = (
                torch.sqrt(var / (nsamples - 1.0)).to("cpu").item()
            )
        if points:
    ...
    def sample_with_context(self, context: np.ndarray, inverse: bool = False):
        ...
        if inverse:
            z = torch.tensor(context[:, 8:]).to(
                self.device
            )  # May be nan for some values. Check it on Hybrid side
            z[:, 0] = z[:, 0] / (2 * np.pi)  # phi
            ...
            list(
                map(
                    lambda x: self.z_mapper.update({x[1].tobytes(): x[0]}),
                    zip(z, context[:, [0, 1, 2]]),
                )
            )
            with torch.no_grad():
